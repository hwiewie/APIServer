...
import (
	...
	"github.com/hwiewie/APIServer/controllers/openapi"
	"github.com/hwiewie/APIServer/models"
	"github.com/hwiewie/APIServer/util/logs"
	"github.com/hwiewie/APIServer/workers"
)
...
func (a NamespaceUserAuditor) Audit(data *message.RequestMessageData) error {
	...
	case "Delete":
		...
		log.Message = fmt.Sprintf("刪除部門用戶（%s）", data.URI)
	}
...
func (a AppAuditor) Audit(data *message.RequestMessageData) error {
	...
	case "Delete":
		log.LogType = models.AuditLogTypeDelete
		log.LogLevel = models.AuditLogLevelWarning
		log.Message = fmt.Sprintf("刪除項目用戶（%s）", data.URI)
	}
...
func (a FallbackRequestAuditor) Audit(data *message.RequestMessageData) error {
	// 如果URLMapping中刪除操作沒有聲明為Delete，那審計時無法判斷是否需要默認記錄該行為
