...
// DeploymentCache 為本地 deployment cache
//   Tank 水箱， 為 Deployment 的容器
//   Tank 的數據結構為 map[namespace]map[name]*v1.Deployment
//   外部使用 sync.map 管理不同 namespace， 內部使用 map[name]*v1.Deployment 不同 deployment。
...
func (d *DeploymentCache) ListDeployments(ctx context.Context, namespace string) ([]appsv1.Deployment, error) {
	...
	// 返回非指針對象，兼容 appsv1.DeploymentList.Items()
